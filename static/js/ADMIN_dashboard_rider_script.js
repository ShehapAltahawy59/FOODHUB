                        ...
                        if (confirmDelete) {
                            ...
                            constantsRef.limit(1).get().then((querySnapshot) => {
                                if (!querySnapshot.empty) {
                                    ...
                                    // Filter out the entry where the name matches ride.name
                                    for (let key in deliveryMenMap) {
                                        if (deliveryMenMap[key].name === rider.name) {
                                            delete deliveryMenMap[key]; // Remove the specific entry
                                            break;
                                        }
                                    }
                        
                                    // Update Firestore with the modified map
                                    doc.ref.update({ delivery_men: deliveryMenMap })
                                        .then(() => {
                                            alert('Record deleted successfully');
                        ...
                        saveBtn.addEventListener('click', function () {
                            ...
                            constantsRef.get().then((querySnapshot) => {
                                if (!querySnapshot.empty) {
                                    ...
                                    // Use Firestore's dot notation to update a specific field in a map
                                    let updateData = {};
                                    updateData[`delivery_men.${rider.phone}`] = updatedRider;
